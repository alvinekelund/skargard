...
function animate() {
  ...
  if ((perfT - lastLocCheck) > 0.5) {
    ...
    // soundscape context (twice a second): shore proximity and woodedness feed
    // the soft wash and leaf voices; engine state feeds the subdued auxiliary.
    ...
    audio.setEnv({ shore, wooded, motorOn: boat.state.motorOn, throttle: boat.state.throttle });
  }
